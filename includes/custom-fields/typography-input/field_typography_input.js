...
(function( $ ) {
    ...
    redux.field_objects.typography_input = redux.field_objects.typography_input || {};
    ...
    $( document ).ready(
        function() {
            //redux.field_objects.typography_input.init();
        }
    );

    redux.field_objects.typography_input.init = function( selector, skipCheck) {


        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-typography_input:visible' );
        }
                    ...
                    function() {
                        ...
                        $( this ).find( '.redux-typography-container' ).each(
                            function() {
                                ...
                                redux.field_objects.typography_input.select( family, true );
                        ...
                        $( this ).find( '.redux-typography' ).on(
                            'change', function() {
                                redux.field_objects.typography_input.select( $( this ) ); 
                            }
                        );
                        ...
                        $( this ).find( '.redux-typography-size, .redux-typography-height, .redux-typography-word, .redux-typography-letter, .redux-typography-align, .redux-typography-transform, .redux-typography-font-variant, .redux-typography-decoration' ).keyup(
                            function() {
                                redux.field_objects.typography_input.select( $( this ).parents( '.redux-container-typography_input:first' ) );
                            }
                        );
                        ...
                        $( this ).find( ".redux-typography-family" ).select2(
                            ...
                            "select2-opening", function( e ) {
                                ...
                                // Get field ID
                                var thisID = $( this ).parents( '.redux-container-typography_input:first' ).attr( 'data-id' );
                                ...
                                if ( isUserFonts == 1 ) {
                                    var fontKids = [];
                                     ...
                                     for ( var key in redux.typography_input[thisID] ) {
                                        var obj = redux.typography_input[thisID].std_font;
                        ...
                        if ( !xx.hasClass( 'redux-typography-family' ) ) {
                            ...
                            el.find( ".redux-typography-weight" ).select2( default_params );
                        }
    ...
    // Return font size
    redux.field_objects.typography_input.size = function( obj ) {
        var size = 0,
            key;
    ...
    redux.field_objects.typography_input.makeBool = function( val ) {
    ...
    redux.field_objects.typography_input.contrastColour = function( hexcolour ) {
    ...
    redux.field_objects.typography_input.select = function( selector, skipCheck ) {
        var mainID;
        
        // Main id for selected field
        mainID = $( selector ).parents( '.redux-container-typography_input:first' ).attr( 'data-id' );
        if (mainID === undefined) {
            mainID = $(selector).attr( 'data-id' );
        }
        

        var parent = $( selector ).parents( '.redux-container-typography_input:first' );
        var data = [];
        ...
        var weight = $( '#' + mainID + ' select.redux-typography-weight' ).val();
        ...
        if ( !$( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            ...
            weight = $( '#' + mainID + ' select.redux-typography-weight' ).data( 'value' );
        ...
        if ( $( selector ).hasClass( 'redux-typography-family' ) ) {
            ...
            if ( details ) {
                $.each(
                    details, function( index, value ) {
                        if ( index === style || index === "normal" ) {
                            ...
                            $( '#' + mainID + ' .typography-weight .select2-chosen' ).text( value );
                        } else {
                            selected = "";
                        }
                ...
                // Destory select2
                $( '#' + mainID + ' .redux-typography-weight' ).select2( "destroy" );

                // Insert new HTML
                $( '#' + mainID + ' .redux-typography-weight' ).html( html );

                // Init select2
                $( '#' + mainID + ' .redux-typography-weight' ).select2( default_params );
        ...
        // Check if the selected value exists. If not, empty it. Else, apply it.
        if ( $( '#' + mainID + " select.redux-typography-weight option[value='" + weight + "']" ).length === 0 ) {
            weight = "";
            $( '#' + mainID + ' select.redux-typography-weight' ).select2( 'val', '' );
        } /*else if ( weight === "400" ) {
            $( '#' + mainID + ' select.redux-typography-weight' ).select2( 'val', weight );
        }*/
        ...
        if ( $( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            ...
            if ( color ) {
                ...
                $( '#' + mainID + ' .typography-preview' ).css(
                    'background-color', redux.field_objects.typography_input.contrastColour( color )
                );
            }

            $( '#' + mainID + ' .typography-style .select2-chosen' ).text( $( '#' + mainID + ' .redux-typography-style option:selected' ).text() );
            $( '#' + mainID + ' .typography-weight .select2-chosen' ).text( $( '#' + mainID + ' .redux-typography-weight option:selected' ).text() );
