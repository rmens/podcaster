...
/* Host Settings */
$pod_host_active = pod_theme_option( 'pod-single-header-author' ); ?>
		...
		<footer class="entry-meta clearfix">
			<div class="entry-taxonomy">
				<?php if ( comments_open() && isset ( $pod_comm_display ) && $pod_comm_display == TRUE ) : ?>
					<span class="comment-link"><?php comments_popup_link( '<span class="leave-reply">' . $pod_blog_comm_0_txt . '</span>', $pod_blog_comm_1_txt, '% ' . $pod_blog_comm_mul‚_txt ); ?></span>
				<?php else : ?>
		...
		<footer class="entry-meta <?php echo esc_attr( $pod_host_active ); ?> clearfix">
			...
			<?php if ( is_singular() ) : // If a user has filled out their description, show a bio on their entries. ?>
				<div class="author-info">
					...
					<?php if( $pod_plugin_active == "ssp") { ?>
						<?php $ssp_speakers_active = pod_theme_option('pod-ssp-guest-active');
